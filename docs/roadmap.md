...
- `/profile` にプロフィール、スキル、経歴を表示する。
- スキルはカテゴリ別に、カラーロゴと技術名を組み合わせて表示する。
- 経歴は Nuxt UI の `UTimeline` を使い、会社ごとのプロジェクトを掲載する。
- ヘッダーからお問い合わせを削除し、プロフィールへの導線を追加する。
- トップページの記事と読了本は、それぞれ最新 2 件を 2 列で表示する。
- 各ページのコンテンツ幅、ヘッダー下の余白、見出しサイズを統一する。
- パンくずリストは使用せず、詳細ページには一覧へ戻る導線を置く。
- ライトモードとダークモードでヘッダーロゴを切り替える。
